...
(function (root) {
  ...
  // 拆牌代价：这手把手里某点数的牌只用了一部分(拆散原结构)→罚。尤其别拆炸弹(留作后期杀招)、别拆对打单。
  function breakCost(m, hand, level) {
    if (isBomb(m.combo)) return 0;                  // 这手本身就是炸/同花顺，不算拆
    const handCnt = {}, useCnt = {};
    for (const c of hand) if (!GD.isJoker(c) && !GD.isWild(c, level)) handCnt[c.rank] = (handCnt[c.rank] || 0) + 1;
    for (const c of m.cards) if (!GD.isJoker(c) && !GD.isWild(c, level)) useCnt[c.rank] = (useCnt[c.rank] || 0) + 1;
    let cost = 0;
    for (const r in useCnt) {
      const have = handCnt[r], use = useCnt[r];
      if (use < have) {                             // 没全用完→拆散了
        if (have >= 4) cost += 25;                  // 拆炸弹！重罚——留着炸弹做后期杀招/夺权
        else if (have === 3) cost += 5;             // 拆三条
        else if (have === 2 && use === 1) cost += 4; // 拆对打单（除非单牌能控/外面没单，一般别拆）
      }
    }
    return cost;
  }
  ...
  function buildMemory(state, seat, tier, level) {
    ...
    return { acc, tier, level, opps, oppCnts, dumped,
             ...
             downCnt: state.hands[(seat + 1) % 4].length };   // 下家(紧邻对手)牌数：少→易顺小牌，该顶
  }
  ...
  function chooseLead(hand, level, state, seat, mem) {
    ...
    for (const m of pool) {
      ...
      else if (t === T.PAIR && k <= 6) sc += 1;                    // 小对(≤6)也早处理，别留成死牌
      sc -= wasteCost(m, level, k);                               // 别把级牌/逢人配当廉价配角(三带二的"二"、顺子填缺等)
      sc -= breakCost(m, hand, level);                            // 别拆炸弹/拆三条/拆对打单
  ...
  function pickFollow(hand, level, nb, mem) {
    ...
    //   例外：下家(紧邻对手)牌已不多→pass 很可能让它顺着走小牌；此时即便只剩大牌可压，也照压不让顺。
    if (best && !isControl(best.combo, mem) && best.cards.length <= 2 && best.combo.key >= FOLLOW_KEY
        && mem.oppMin > FOLLOW_OPP && hand.length > 5 && mem.downCnt > DOWN_RUN) return null;
    return best;
  }
  ...
  const FOLLOW_KEY = 7, FOLLOW_OPP = 3;                       // 跟牌纪律阈值(扫参定档:既顶小牌又保记忆梯度)
  const DOWN_RUN = 4;                                        // 下家≤此牌数→视为"很可能走小牌",即便只剩大牌也顶住不让顺
